-- Fix ambiguity in approve_teacher_availability_atomic:
-- RETURNS TABLE output column names (class_option_id/class_slot_id) can collide
-- with ON CONFLICT column inference in PL/pgSQL variable resolution.
-- Keep behavior unchanged; only disambiguate references.
...
begin
  ...
  begin
    ...
    select
      true as ok,
      v_co_id as class_option_id,
      v_cs_id as class_slot_id,
      null::text as error;
  exception when others then
    ...
    select
      false as ok,
      null::text as class_option_id,
      null::text as class_slot_id,
      sqlerrm as error;
  end;
end;
$$;
