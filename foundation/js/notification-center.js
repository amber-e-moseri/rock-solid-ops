...
function normalizeMoodleRows(rows) {
  return rows.map((r) => ({
    ...
    recipient: r.student_email || "",
    subject: `Student ${r.student_id || ""}`,
    action: "Moodle Sync",
    error: r.error_message || r.last_error || "",
    created_at: r.updated_at || r.created_at || r.synced_at,
    raw: r,
  }));
}
...
async function loadData() {
  ...
  try {
    const [emailRows, scheduledRows, failedSyncRows, moodleRows] = await Promise.all([
      ...
      selectSafe("moodle_sync", (q) =>
        q.select("id,student_id,status,error_message,last_error,updated_at,created_at,synced_at")
...
async function invokeRetryWorker(action, source, id) {
  ...
  const { data, error } = await supabase.functions.invoke("retry-worker", { body: payload });
  if (error) throw error;
